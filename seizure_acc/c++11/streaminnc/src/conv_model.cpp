...
Model::Model(int inputLength, int bufferLength, int bufferDepth):
    ...
    // Initialize layers
    this->conv1 = Conv1D(8, 3, this->inputLength, 4);
    this->bn1 = BatchNormalization(8);

    this->conv2 = Conv1D(16, 3, this->inputLength, 8);
    this->bn2 = BatchNormalization(16);

    this->conv3 = Conv1D(32, 3, this->inputLength, 16);
    this->bn3 = BatchNormalization(32);

    this->conv4 = Conv1D(64, 3, this->inputLength, 32);
    this->bn4 = BatchNormalization(64);

    this->conv5 = Conv1D(128, 3, this->inputLength, 64);
    this->bn5 = BatchNormalization(128);

    this->conv6 = Conv1D(256, 3, this->inputLength, 128);
...
void Model::resetPredictionBuffer(){
    this->initializeBuffer();
}
...
void Model::addToBuffer(const std::vector<std::vector<double>>& x){
