...
for recording in recordings:
    ...
    with tf.device("/:cpu0"):
        ...
        for i in tqdm(samples_indexes):
            ...
            mask = np.zeros(tf.shape(output1))
            mask[:, -sample_shift:, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            ...
            mask = np.zeros(tf.shape(output2))
            mask[:, -sample_shift:, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            ...
            mask = np.zeros(tf.shape(output3))
            mask[:, -sample_shift:, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            
            output_tmp4 = conv4(output3 * mask)
            ...
            output_tmp5 = conv5(output4 * mask)
            output5 = tf.concat([prev_act_5[:, sample_shift:, :], 
                                  output_tmp5[:, -sample_shift:, :]], axis = 1)
            
            mask = np.zeros(tf.shape(output5))
            mask[:, -sample_shift:, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            
            output_tmp6 = conv6(output5 * mask) 
    ...
    output_file_name += recording + '_approximate.pickle'
