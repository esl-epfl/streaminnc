...
for test_subject_id in range(1, 16):
    ...
    X_validate = X_validate[::4, ...].flatten()
    y_validate = y_validate[::4, ...].flatten()
    y_validate = np.repeat(y_validate, 256)

    window_length = 256
    window_step = window_length // 4 - 1

    Xs = []
    ys = []
    for i in range(0, X_validate.shape[0], window_step):
        
        if i + window_length > X_validate.shape[0]:
            break
        
        Xs.append(X_validate[i : i + window_length])
        ys.append(y_validate[i : i + window_length].mean())
        
    X_validate = np.stack(Xs)[:, None, :]
    y_validate = np.stack(ys)
    ...
    results = {'y_test' : y_validate.flatten(),
                'y_pred' : y_pred.flatten(),
                'y_pred_shift' : y_pred_shift.flatten()}
        
    output_file_name = './results/zero_padding_pooling_misalignment/S' + str(test_subject_id) + '.pickle'
