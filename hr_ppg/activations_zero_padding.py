...
plt.plot( X_emb_2[index, 8:, channel], linewidth = linewidth)
plt.plot(X_emb_2[index + 1, :-8, channel], linewidth = linewidth)

plt.xlabel('Activation sample')
plt.ylabel('Activation value')

if save_figures:
    plt.savefig('./results/figures/zero_padding_effects/zero_padding_activations.svg', 
                bbox_inches = 'tight')
    
print(get_nrmse(X_emb_2[index, 8:, channel], 
                X_emb_2[index + 1, :-8, channel]) * 100)
