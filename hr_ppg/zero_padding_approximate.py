...
for test_subject_id in range(1, 16):
    ...
    with tf.device("/:cpu0"):
        ...
        for i in tqdm(samples_indexes):
            ...
            mask = np.zeros(tf.shape(output4_pool))
            mask[:, -int(sample_shift/4):, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            

            output_tmp4 = conv21(output4_pool * mask)
            output4 = tf.concat([prev_act_4[:, int(sample_shift / 4):, :], 
                                output_tmp4[:, -int(sample_shift / 4):, :]], axis = 1)
            
            mask = np.zeros(tf.shape(output4))
            mask[:, -int(sample_shift/4):, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            
            output_tmp5 = conv22(output4 * mask)
            output5 = tf.concat([prev_act_5[:, int(sample_shift / 4):, :], 
                                output_tmp5[:, -int(sample_shift / 4):, :]], axis = 1)
            
            output_tmp6 = conv23(output5 * mask)
            ...
            mask = np.zeros(tf.shape(output7_pool))
            mask[:, -int((sample_shift / 4) / 2):, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            
            output_tmp7 = conv31(output7_pool * mask)
            output7 = tf.concat([prev_act_7[:, int((sample_shift / 4) / 2):, :], 
                                output_tmp7[:, -int((sample_shift / 4) / 2):, :]], axis = 1)
            
            mask = np.zeros(tf.shape(output7))
            mask[:, -int((sample_shift / 4) / 2 ):, :] = 1
            mask = tf.constant(mask, dtype = tf.float32)
            
            output_tmp8 = conv32(output7 * mask)
            output8 = tf.concat([prev_act_8[:, int((sample_shift / 4) / 2):, :], 
                                output_tmp8[:, -int((sample_shift / 4) / 2):, :]], axis = 1)
            
            output_tmp9 = conv33(output8 * mask)
    ...
    output_file_name = './results/zero_padding_approximate/S' + str(test_subject_id) + '.pickle'
